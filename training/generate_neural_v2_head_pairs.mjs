...
function actionForHead(head, valueIndex) {
  const raw = {head, source: "paired-v2-single-head"};
  raw[`${head}Index`] = valueIndex;
  return Object.freeze({...normalizeNeuralV2Action(raw), head, isolated: true});
}

export function createNeuralV2HeadPlan(seed, level, durationMs, headIndex = 0) {
  const random = seededRandom(seed);
  const head = HEADS[Math.max(0, Math.min(HEADS.length - 1, Math.floor(headIndex)))];
  const classes = HEAD_CLASSES[head];
  const valueIndex = Math.min(classes.length - 1, Math.floor(random() * classes.length));
  const maximumStart = threatAwareStartWindow(level, durationMs);
  return {
    head,
    headIndex: HEADS.indexOf(head),
    valueIndex,
    value: classes[valueIndex],
    action: actionForHead(head, valueIndex),
    ...
    isolatedHeadFramesBeforeLastSample: null,
    isolatedHeadFramesAtEnd: null,
  };
}

function candidatesForHead(actors, head) {
  if (head === "fire") return actors.filter(actor => actor.controlsFire !== false);
  if (head === "route") return actors.filter(actor => actor.controlsMovement !== false && actor.kind === "boat");
...
function startPlanIfReady(server, plan, elapsedMs) {
  ...
  const candidates = candidatesForHead(collectNeuralActors(server.world), plan.head);
  ...
  setServerNeuralV2Override(server, actor.id, {...plan.action, head: plan.head});
}
...
function samplePlan(server, plan, elapsedMs, samples) {
  ...
  plan.controlledFramesBeforeLastSample = Number(diagnostics.controlledFrames) || 0;
  plan.isolatedHeadFramesBeforeLastSample = Number(diagnostics.isolatedHeadFrames?.[plan.head]) || 0;
  samples.push({
    t: elapsedMs,
    actorId: actor.id,
    role: actor.role,
    ...
    head: plan.head,
    headIndex: plan.headIndex,
    value: plan.value,
    valueIndex: plan.valueIndex,
  });
...
function finishPlanAfterTick(server, plan, elapsedMs) {
  ...
  const diagnostics = neuralV2OverrideStatus(server).diagnostics;
  plan.controlledFramesAtEnd = Number(diagnostics.controlledFrames) || 0;
  plan.isolatedHeadFramesAtEnd = Number(diagnostics.isolatedHeadFrames?.[plan.head]) || 0;
...
async function simulateRollout({battleIndex, worldSeed, durationMs, level, script, coop, plan}) {
  return withWorldRandomSeed(worldSeed, async () => {
    const startedAt = 6_000_000 + battleIndex * (durationMs + 5000);
    ...
    return {outcome, elapsedMs, playerHealth, boatHull, boatWater, enemyHits, score, diagnostics, samples};
  });
}

export async function simulateNeuralV2HeadPair({battleIndex, durationMs, level, script, coop}) {
  const relativeHeadIndex = Math.abs(Math.floor(battleIndex)) % HEADS.length;
  const worldSeed = 18_725_552 + battleIndex * 10_007;
  const planSeed = 19_925_552 + battleIndex * 65_537;
  const baseline = await simulateRollout({battleIndex, worldSeed, durationMs, level, script, coop, plan: null});
  const plan = createNeuralV2HeadPlan(planSeed, level, durationMs, relativeHeadIndex);
  const explored = await simulateRollout({battleIndex, worldSeed, durationMs, level, script, coop, plan});
  return {
    id: `v2-head-pair-${battleIndex}-${worldSeed}`,
    battleIndex,
    seed: worldSeed,
    planSeed,
    level,
...
function usablePair(pair, minimumAdvantage) {
  return pair.advantage >= minimumAdvantage
    ...
    && Number(pair.intervention?.isolatedHeadFramesAtEnd) > Number(pair.intervention?.isolatedHeadFramesBeforeLastSample)
    && pair.explored?.samples?.length >= 2;
}

export function selectNeuralV2HeadElites(pairs, perGroup = 1, minimumAdvantage = 2.5) {
  const groups = new Map();
  for (const pair of pairs) {
    if (!usablePair(pair, minimumAdvantage)) continue;
    const key = `${pair.level}:${pair.script}:${pair.coop ? "coop" : "solo"}:${pair.intervention.actorRole || "unknown"}:${pair.head}:${pair.valueIndex}`;
  ...
  return selected.sort((left, right) => left.head.localeCompare(right.head) || left.level - right.level || right.advantage - left.advantage);
}
...
async function main() {
  ...
  const output = argument("output", `training/reports/v2-head-pairs/shard-${shard}.json`);
  const pairs = [];
  ...
  for (let battleIndex = startIndex + shard; battleIndex < endIndex; battleIndex += shards) {
    ...
    pairs.push(pair);
    addCount(baselineOutcomes, pair.baseline.outcome);
    addCount(exploredOutcomes, pair.explored.outcome);
    addCount(headPairCounts, pair.head);
    addCount(valuePairCounts, `${pair.head}:${pair.valueIndex}`);
    if (usablePair(pair, minimumAdvantage)) {
      addCount(positiveByHead, pair.head);
      addCount(positiveByValue, `${pair.head}:${pair.valueIndex}`);
    }
  }

  const elitePairs = selectNeuralV2HeadElites(pairs, elitePerGroup, minimumAdvantage);
  const advantages = pairs.map(pair => pair.advantage);
  const sampledPairs = pairs.filter(pair => (pair.explored?.samples?.length || 0) >= 2);
  const allSamples = pairs.flatMap(pair => pair.explored?.samples || []);
  const completed = pairs.filter(pair => pair.intervention?.completed
    && Number(pair.intervention?.appliedSamples) === Number(pair.intervention?.durationSamples));
  const finalTickProofPairs = completed.filter(pair => finalTickProved(pair.intervention)).length;
  const isolatedHeadProofPairs = completed.filter(pair => Number(pair.intervention?.isolatedHeadFramesAtEnd)
    > Number(pair.intervention?.isolatedHeadFramesBeforeLastSample)).length;
  ...
  const integrity = {
    ...
    sampledFrames: sampledPairs.reduce((sum, pair) => sum + pair.explored.samples.length, 0),
    ...
    finalTickProofPairs,
    invalidFinalTickProofs: completed.length - finalTickProofPairs,
    isolatedHeadProofPairs,
    invalidIsolatedHeadProofs: completed.length - isolatedHeadProofPairs,
    waterGuardInterventions: pairs.reduce((sum, pair) => sum + (Number(pair.explored?.diagnostics?.waterGuardInterventions) || 0), 0),
  };
  const positivePairs = pairs.filter(pair => usablePair(pair, minimumAdvantage)).length;
  const report = {
    ...
    headSchema: Object.fromEntries(HEADS.map(head => [head, HEAD_CLASSES[head]])),
    critique: [
      "Each explored rollout changes exactly one named v2 head over the unchanged v1 controller.",
      "The other four v2 heads receive no labels and do not replace v1 movement or fire.",
      "The unchanged and isolated-head rollouts share one production world seed.",
      "Every retained sample is followed by an authoritative server tick and records which head and value were tested.",
      "This batch discovers causal head examples only; it does not train or enable a v2 model.",
    ],
    elitePairs,
  };
