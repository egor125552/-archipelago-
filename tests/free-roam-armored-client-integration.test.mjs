...
import {
  ...
  resolveBoatPhysicsProfile,
} from "../public/src/free-roam-boat-physics.js";
...
function predictedWorld(physicsProfile = null) {
  const boat = {
    ...
    speed: 0,
    throttle: 0,
    collisionRadius: physicsProfile ? 7.5 : 6,
    physicsProfile: physicsProfile ? {...physicsProfile} : undefined,
    engineStalled: false,
...
test("armored mini-physics stays inside the common profile law", () => {
  const standard = resolveBoatPhysicsProfile({}, {maxSpeed: 21, reverseSpeed: -6, acceleration: 8.4, drag: 0.09});
  const armored = resolveBoatPhysicsProfile({physicsProfile: DUAL_TURRET_PHYSICS_PROFILE}, {maxSpeed: 21, reverseSpeed: -6, acceleration: 8.4, drag: 0.09});
  ...
  applyBoatPhysicsProfiles(world, before, 0.05, {
    tuning: {maxSpeed: 21, reverseSpeed: -6, acceleration: 8.4, drag: 0.09},
  });
...
test("client prediction uses the same generic profile and feels heavier", () => {
  ...
  assert.ok(Math.hypot(armored.boats[0].x - 210, armored.boats[0].y - 210)
    < Math.hypot(ordinary.boats[0].x - 210, ordinary.boats[0].y - 210));
});
...
test("exit keeps boat identity for merchant and armored boarding audio", () => {
  const world = {
    ...
    events: [{type: "exit", text: "Ты вышел на берег.", targets: [0]}],
  };
  ...
  const played = [];
  const handled = handleDualTurretAudioEvent({
...
test("integration source has no armored-only parallel physics or owner-only merchant rule", async () => {
  ...
  assert.match(audio, /startsWith\("dual-turret"\)/);
  assert.match(audio, /event\.boatType === DUAL_TURRET_BOAT_TYPE/);
