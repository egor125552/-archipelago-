...
test("outdoor explosion uses image-source water and shore paths, not room reverb", () => {
...
test("reflection planning falls back safely when listener geometry is missing", () => {
...
test("client uses only supplied recordings and contains no synthesised audio", async () => {
  ...
  assert.match(source, /mega-bomb-real-v2\/flight\.mp3/);
  assert.match(source, /mega-bomb-real-v2\/explosion\.mp3/);
  ...
  assert.doesNotMatch(source, /DelayNode|createDelay/);
});
