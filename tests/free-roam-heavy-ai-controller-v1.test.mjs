...
function makeWorld() {
  const boat = {
    ...
    turretHealth: 240, maxTurretHealth: 240, engineDisabled: false,
    turretDisabled: false, fireCooldown: 1, burstRemaining: 0,
    burstCooldown: 0, aimRemaining: 0, targetPlayer: 0,
  };
  return {
    time: 100, events: [],
    players: [{x:30,y:30,heading:0,mode:"foot",combat:{alive:true,equipped:"automatic",lockedTargetId:"heavy-turret",lastTargetRequestId:"heavy-turret"}}],
    boats: [],
    freeActivities: {presence:[true],inputs:[{targetId:"heavy-turret"}]},
...
test("old repair phases migrate into the single canonical state machine", () => {
  for (const [legacy, expected] of [["retreating","escape"],["breach-escaping-v166","escape"],["stopping-v165","stopping"],["breach-repairing-v166","repairing"],["breach-returning-v166","returning"]]) {
    const heavy = {phase: legacy};
    assert.equal(normalizeHeavyPhaseV1(heavy), expected);
    assert.equal(heavy.phase, expected);
  }
});
...
test("destroyed turret starts one repair escape", () => {
  ...
  heavy.armourBreached=true;heavy.phase="combat";
  world.freeHeavyPursuer.boat.turretHealth=0;world.freeHeavyPursuer.boat.turretDisabled=true;
...
test("a close player aborts turret repair and no later layer resurrects it", () => {
  ...
  Object.assign(world.freeHeavyPursuer.boat,{turretHealth:0,turretDisabled:true,x:150,y:150});
  Object.assign(world.players[0],{x:155,y:150});
  finishHeavyAiControllerV1(world,0.1);
...
test("engine destruction during an escape always takes priority and stops the boat", () => {
  ...
  Object.assign(world.freeHeavyPursuer.boat,{turretHealth:0,engineHealth:0,engineDisabled:true});
  finishHeavyAiControllerV1(world,0.1);
...
test("a living heavy target remains locked for a long-range mega-bomb", () => {
  ...
  const state=world.freeHeavyAiControllerV1,eventStart=world.events.length;
  world.events.push({type:"target-cleared",targets:[0],at:world.time});
  preserveHeavyTargetLocksV1(world,state,eventStart);
  assert.equal(world.players[0].combat.lockedTargetId,"heavy-turret");
...
test("an old heavy from another encounter is removed with crew, shots and locks", () => {
  const world=makeWorld();prepareHeavyAiControllerV1(world);
  world.freeHostileActors.actors.push({id:"old-heavy-crew",boatId:"heavy-pursuer",active:true});
  world.freeHeavyPursuer.projectiles.push({id:"old-heavy-shot"});world.freeThreatDirector.encounterId=2;
  assert.equal(retireStaleHeavyV1(world),true);
  assert.equal(world.freeHeavyPursuer.boat,null);assert.deepEqual(world.freeHeavyPursuer.projectiles,[]);
