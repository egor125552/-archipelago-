...
test("a low aft blast on the fast boat damages lower-deck machinery but not the pistol above", () => {
  ...
  const result = applySpatialVesselImpact(world, entry, {
    ...
    impactPoint,
    impactHeight: 0.7,
    projectileId: "test-low-aft-blast",
  });
...
test("a high control-deck blast can damage the fast boat pistol installation", () => {
  ...
  const result = applySpatialVesselImpact(world, entry, {
    ...
    impactHeight: 2.15,
    fromAbove: true,
    projectileId: "test-control-deck-blast",
  });
...
test("armored patrol spends exterior armor before the selected compartment and module", () => {
  ...
  const result = applySpatialVesselImpact(world, entry, {
    ...
    impactPoint,
    impactHeight: 0.7,
    sourcePoint: vesselLocalToWorld(boat, {x: 14, y: 1.2}),
    projectileId: "test-armored-side-hit",
  });
...
test("an internal blast damages occupants on its deck instead of every person aboard", () => {
  ...
  lower.mode = "boat";
  lower.activeBoat = boat.id;
  lower.combat.health = 100;
  lower.combat.alive = true;
  upper.mode = "boat";
  upper.activeBoat = boat.id;
  upper.combat.health = 100;
  upper.combat.alive = true;
  boat.crew = [0, 1];
  ...
  const result = applySpatialVesselImpact(world, entry, {
    ...
    damage: 24,
    playerDamage: 18,
    impactPoint: vesselLocalToWorld(boat, {x: 0, y: -2.3}),
    impactHeight: 0.7,
    projectileId: "test-crew-deck-isolation",
  });
