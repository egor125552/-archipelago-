...
test("both players spawn nearby in separate boats", () => {
  const world = createFreeWorld();
  const [first, second] = world.boats;
  const metres = Math.hypot(first.x - second.x, first.y - second.y);
  assert.ok(metres >= 16 && metres <= 24);
  assert.deepEqual(world.players.map(player => player.mode), ["boat", "boat"]);
  assert.deepEqual(world.players.map(player => player.activeBoat), [0, 1]);
});
