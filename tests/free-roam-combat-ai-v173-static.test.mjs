...
test("authoritative hotfix uses one heavy controller instead of the V164-V176 layer chain", () => {
  ...
  const combat = fs.readFileSync(new URL("../public/src/free-roam-combat.js", import.meta.url), "utf8");
  ...
  const damage = fs.readFileSync(new URL("../public/src/free-roam-heavy-automatic-damage-v1.js", import.meta.url), "utf8");

  assert.match(hotfix, /free-roam-heavy-ai-controller-v1\.js\?v=5/);
  assert.match(hotfix, /free-roam-heavy-hull-damage-memory-v1\.js\?v=1/);
  assert.match(combat, /free-roam-heavy-pursuer\.js\?v=4/);
  assert.match(controller, /FIELD_REPAIR_SECONDS=7/);
  assert.match(controller, /heavy\.phase==="returning"/);
  assert.match(damage, /heavyAutomaticDamageScaleV1/);
  ...
  assert.doesNotMatch(damage, /heavy\.phase\s*=/);
});
