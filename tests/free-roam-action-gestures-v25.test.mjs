...
test("two-finger gestures control sonar, pump, repair and weapon without disabling gesture mode", () => {
  ...
  assert.equal(classifyActionGesture({pointers: 2, duration: 340, dx: 5, dy: 90, movement: 92}), null);
});
...
test("four fingers request a one-shot sonar heading turn", () => {
  assert.equal(classifyActionGesture({pointers: 4, duration: 180, dx: 2, dy: 2, movement: 5}), "guide");
  assert.equal(classifyActionGesture({pointers: 5, duration: 180, dx: 2, dy: 2, movement: 5}), null);
});
...
test("the iPhone gesture area covers the game section and documents every action", async () => {
  ...
  assert.match(html, /Четыре пальца/);
