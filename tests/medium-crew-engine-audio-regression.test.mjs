...
const audioV4 = readFileSync(new URL("../public/src/free-roam-audio-v4.js", import.meta.url), "utf8");
...
test("medium crew ship cannot fall through to the ordinary local light-boat engine", () => {
  assert.match(audioV2, /profile\.startsWith\("medium-crew"\)/);
  assert.match(audioV2, /!hasDedicatedVesselEngine\(localBoat\)/);
});

test("medium crew ship cannot fall through to the ordinary remote light-boat engine", () => {
...
test("release 1.7.5 keeps stale-cache protection and forces a fresh remote-audio layer", () => {
  ...
  assert.match(worker, /free-roam-audio-v4\.js\?v=42[\s\S]*?free-roam-audio-v4\.js\?v=43/);
