...
export function createGame(options = {}) {
  ...
  return {
    ...
    boat: {
      x: 0, y: 0, heading: 0, speed: 0, throttle: 0, rudder: 0,
      hull: 100, water: 0, leak: 0, fuel: 100, engineTemp: 24,
      engineStalled: false, pumpActive: false, rescueActive: false, repairProgress: 0,
    },
    controls: {left: false, right: false, forward: false, reverse: false, pump: false, rescue: false},
    sonar: {cooldown: 0, lastResult: null, pings: 0},
    crew: {aiEnabled: mode === "solo", task: "watch", confidence: 0.65},
    rescued: 0,
    collisions: {},
    eventLog: [],
...
export function sonarPing(state) {
  ...
  const result = {kind: nearest.target.kind, id: nearest.target.id, distance: nearest.distance, relativeAngle: relative, pan};
...
export function getView(state) {
  ...
  return {
    phase: state.phase, mode: state.mode, role: state.role, message: state.message,
    elapsed: state.elapsed, remaining: Math.max(0, CONFIG.missionDuration - state.elapsed),
    rescued: state.rescued, score: state.score, won: state.won, lost: state.lost, ending: state.ending,
    boat: {
      heading: boat.heading, speed: boat.speed, throttle: boat.throttle, hull: boat.hull,
      water: boat.water, leak: boat.leak, fuel: boat.fuel, engineTemp: boat.engineTemp,
      engineStalled: boat.engineStalled, pumpActive: boat.pumpActive, repairProgress: boat.repairProgress,
    },
    sonar: {...state.sonar},
