...
function improveTrajectory(projectile) {
  ...
  const wantedCurve = clamp(pathDistance * 0.78, 20, 78);
  ...
  projectile.arcHeight = clamp(18 + pathDistance * 0.12, 21, 36);
  projectile.arcSkew = 0.92 + (idNumber(projectile) % 5) * 0.035;
  projectile.flightTime = clamp(pathDistance / 51 * 1.16, 0.95, 3.9);
  projectile.audioArcSide = chosen.side;
  projectile.audioArcStrength = clamp(chosen.effective / Math.max(12, pathDistance * 0.34), 0.45, 1);
}
