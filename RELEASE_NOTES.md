...
- Редкие опасные контракты пятого уровня вызывают отдельный тяжёлый катер и сопровождение.
- Тяжёлый катер медленно разгоняется, тяжело поворачивает, сильно таранит и имеет отдельные корпус, двигатель и пулемётную установку.
- Пистолет не пробивает его броню; автомат и таран повреждают корпус и системы.
- Отключение двигателя снижает скорость, уничтожение установки прекращает тяжёлые очереди.

## Элитный стрелок

- В тяжёлом катере физически находится элитный автоматчик со 120 единицами здоровья.
- После высадки или уничтожения лодки он продолжает бой на берегу либо в воде.
- Вблизи он заранее предупреждает о сильном ножевом ударе; его скорость ниже бега игрока, поэтому от атаки можно уклониться.

## Награда

- Полная победа на пятой угрозе выдаёт 500 командных кредитов и шесть физических трофейных ящиков.
- Награда серверная и выдаётся ровно один раз, включая переподключение игроков.

## Проверка

- 204 Node-теста покрывают создание тяжёлой группы, броню, компоненты, пулемёт, элитного стрелка и однократную награду.
- Двухклиентные браузерные сценарии и Cloudflare Worker bundle должны пройти в GitHub Actions перед слиянием.
- Пространственный бой на физическом iPhone остаётся ручной проверкой опубликованной версии.
