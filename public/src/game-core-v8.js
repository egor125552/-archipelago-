...
function firstRescueTutorial() {
  return "Как спасти первого человека. Первое: нажми сонар один раз. Второе: коротко дай газ и поворачивай вправо, пока высокий стереописк не станет двойным и не окажется точно по центру. Третье: отпусти руль и иди вперёд; по мере приближения писк станет чаще. Четвёртое: примерно за двадцать метров отпусти газ и тормози. Когда человек ближе двенадцати метров, а скорость ниже четырёх узлов, один раз нажми Подать спасательный трос и больше не нажимай до ста процентов.";
}
...
function segmentCircleEntry(start, end, circle, radius) {
  ...
  const t1 = (-b - root) / (2 * a);
  const t2 = (-b + root) / (2 * a);
  const candidates = [t1, t2].filter(t => t >= 0 && t <= 1).sort((x, y) => x - y);
