...
function spatialState(event, playerIndex, kind) {
  ...
  const lowpass = occluded
    ? (explosion ? 820 : 620)
    : clamp((explosion ? 17400 : 15800) - distance * (explosion ? 54 : 62), explosion ? 1050 : 700, explosion ? 17400 : 15800);
  return {
...
class MegaBombAudio {
  ...
  async load(name, url) {
    ...
    for (let channel = 0; channel < decoded.numberOfChannels; channel += 1) {
      ...
      for (let index = 0; index < output.length; index += 1) output[index] += input[index] / decoded.numberOfChannels;
    }
  ...
  play(name, options = {}) {
    ...
    volume.gain.linearRampToValueAtTime(Math.max(0.0001, gain), now + Math.max(0.004, Number(options.attack) || 0.012));
    source.start(now);
  ...
  async launch(event, playerIndex) {
    ...
    if (!voice) return;
    const remaining = clamp((Number(event.maxAge) || 0) - (Number(event.age) || 0) + 0.8, 0.5, 12);
...
function install() {
  ...
  globalThis.addEventListener(EVENT_NAME, event => {
    ...
    for (const gameEvent of message.events) {
      ...
      else if (gameEvent.type === "mega-bomb-flight" || gameEvent.type === "mega-bomb-ricochet") audio.flight(gameEvent, playerIndex);
      else if (gameEvent.type === "mega-bomb-explosion") audio.explode(gameEvent, playerIndex);
      else if (gameEvent.type === "mega-bomb-explosion-spatial") audio.updateImpact(gameEvent, playerIndex);
