...
import {AudioEngine as V7AudioEngine} from "./audio-engine-v7.js?base=1";
...
export class AudioEngine extends V7AudioEngine {
  ...
  playGuide(view) {
    ...
    const interval = centered ? 0.92 : near ? 0.68 : 0.9;
    this.nextGuidePipAt = this.ctx.currentTime + interval;
  }
  ...
  playHazardGuide(view) {
    ...
    if (metres == null || metres > 27 || Math.abs(angle || 0) > 68) return;
  ...
  handle(events) {
    for (const event of events || []) {
      if (event.type === "capture-ready") {
        ...
        // On-demand reports are conveyed by speech; no additional alert is needed.
      } else {
        super.handle([event]);
      }
    }
