...
export class AudioEngine extends V8AudioEngine {
  ...
  update(view) {
    ...
    if (playing) {
      ...
      this.ensureLoop(idleName, {
        gain: moving ? 0.026 : 0.042,
        rate: moving ? 0.96 : 0.91,
        lowpass: moving ? 3600 : 2700,
      ...
      if (water > 0.7 || leak > 0.05) {
        ...
        this.ensureLoop(bilgeName, {
          gain: 0.012 + openness * 0.235 + Math.min(0.045, leak / 260),
          rate: 0.78 + openness * 0.16,
          lowpass: 480 + openness * 7600,
      ...
      for (const name of ["riverIdle", "riverWake", "bilgeWater", "motorboatReal", "pumpReal"]) this.stopLoop(name);
    }
  ...
  handle(events) {
    for (const event of events || []) {
      ...
      } else if (event.type === "repair-window-start") {
        this.playSynthPip({frequency: 430, gain: 0.08, duration: 0.1});
        this.playSynthPip({frequency: 360, gain: 0.075, duration: 0.12, delay: 0.18});
      } else if (event.type === "repair-window-warning") {
        this.playSynthPip({frequency: event.critical ? 250 : 330, gain: event.critical ? 0.12 : 0.085, duration: 0.16});
