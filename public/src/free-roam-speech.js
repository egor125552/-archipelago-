...
export function createSpeechController({
  ...
  function start(text) {
    ...
    // Chrome's platform voice can occasionally omit onend/onerror. Do not let
    // one stuck Windows utterance block every later game announcement.
    watchdog = setTimeout(() => {
      if (token !== activeToken) return;
  ...
  function speak(text, {interrupt = false} = {}) {
    ...
    if (!interrupt) {
      // Keep only the newest ordinary update. This prevents a backlog of old
      // positions and combat notices from turning small ping into audible lag.
