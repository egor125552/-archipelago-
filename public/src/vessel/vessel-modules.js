...
function installationFor(registry, definition, runtime, request) {
  ...
  if (mounts.length !== moduleType.installation.mountCount) {
    throw new VesselContractError(`module ${id} needs exactly ${moduleType.installation.mountCount} mounts`);
  }
...
export function installVesselModule(registry, definition, runtime, request) {
  ...
  // No runtime mutation occurs until every required mount has passed validation.
...
export function uninstallVesselModule(runtime, moduleInstanceId) {
  ...
  const mounts = [...(installation.mounts || [])];
  // Release the complete installation in one operation; partial uninstall is not a valid state.
  for (const mountId of mounts) {
    if (runtime.mountOccupancy?.[mountId] === id) delete runtime.mountOccupancy[mountId];
  }
