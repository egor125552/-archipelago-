...
import {
  ...
  resolveVesselZoneAt,
  validateVesselOccupantPosition,
...
export function setVesselOccupantPosition(definition, runtime, playerIndex, position) {
  ...
  const normalized = validateVesselOccupantPosition(definition, position);
  runtime.occupants ||= {};
  runtime.occupants[playerIndex] = {...normalized};
...
export function clearVesselOccupantPosition(runtime, playerIndex) {
  ...
  releaseVesselOccupantResources(runtime, playerIndex);
...
export function syncWalkableVesselOccupants(world, definition, runtime, boat) {
  ...
  for (const [rawIndex, local] of Object.entries(runtime.occupants)) {
    ...
    if (!player || player.activeBoat !== boat?.id) {
      if (boat) {
        boat.vesselRuntimeState ||= {version: 1};
        boat.vesselRuntimeState.occupantMemory ||= {};
        boat.vesselRuntimeState.occupantMemory[rawIndex] = {...local};
      }
      clearVesselOccupantPosition(runtime, playerIndex);
      continue;
    }
    const worldPosition = vesselLocalToWorld(boat, local);
    player.x = worldPosition.x;
    player.y = worldPosition.y;
    player.heading = wrap((Number(boat.heading) || 0) + (Number(local.heading) || 0));
    const resolvedZone = resolveVesselZoneAt(definition, local.deckId, local);
    if ((local.zoneId || null) !== (resolvedZone?.id || null)) local.zoneId = resolvedZone?.id || null;
  }
}
...
export function vesselLandmarkGuidance(definition, runtime, playerIndex, landmarkId) {
  ...
  const relativeBearing = wrap(deg(Math.atan2(dx, dy)) - (Number(local.heading) || 0));
  return Object.freeze({
