...
class FreeAudio {
  ...
  createEngines() {
    const local = this.createEngine(0);
    local.gain.connect(this.ctx.destination);
    this.engine = local.oscillator;
    this.engineGain = local.gain;

    const remote = this.createEngine(0);
    const pan = this.ctx.createStereoPanner();
    remote.gain.connect(pan).connect(this.ctx.destination);
    this.remoteEngine = remote.oscillator;
    this.remoteGain = remote.gain;
    this.remotePan = pan;
  }

  burst({duration = .12, gain = .15, frequency = 900, pan = 0} = {}) {
    ...
    source.connect(filter).connect(amp).connect(panner).connect(this.ctx.destination);
    source.start(now);
    source.stop(now + duration);
  }

  tone(frequency, duration, gain = .1, pan = 0) {
    if (!this.ctx) return;
    const oscillator = this.ctx.createOscillator();
    const amp = this.ctx.createGain();
    const panner = this.ctx.createStereoPanner();
    ...
    oscillator.frequency.exponentialRampToValueAtTime(Math.max(30, frequency * .65), now + duration);
    amp.gain.setValueAtTime(gain, now);
    amp.gain.exponentialRampToValueAtTime(.001, now + duration);
    panner.pan.value = clamp(pan, -1, 1);
    oscillator.connect(amp).connect(panner).connect(this.ctx.destination);
  ...
  playEvent(event) {
    ...
    if (event.type === "footstep") this.burst({duration: .08, gain: .12, frequency: 1100});
    else if (event.type === "swim-step" || event.type === "splash") this.burst({duration: .22, gain: .2, frequency: 550});
    else if (event.type === "collision" || event.type === "ram") {
      this.burst({duration: .38, gain: .32, frequency: 180});
      this.tone(90, .28, .22);
    } else if (event.type === "tow-attach") {
      this.tone(240, .22, .12, -.2);
      this.tone(170, .34, .1, .2);
    } else if (event.type === "tow-detach" || event.type === "tow-strain") {
      this.burst({duration: .3, gain: .18, frequency: 380});
    } else if (event.type === "jump" || event.type === "roof") {
      this.tone(360, .14, .09);
    }
  }

  update(currentWorld) {
    if (!this.ctx || !currentWorld) return;
    const me = currentWorld.players[playerIndex];
    const other = currentWorld.players[1 - playerIndex];
    const myBoat = me?.mode === "boat" ? currentWorld.boats[me.activeBoat] : null;
    const otherBoat = other?.mode === "boat" || other?.mode === "roof" ? currentWorld.boats[other.activeBoat] : null;
    const now = this.ctx.currentTime;
    const localSpeed = Math.abs(myBoat?.speed || 0);
    this.engine.frequency.setTargetAtTime(45 + localSpeed * 4.2, now, .08);
    this.engineGain.gain.setTargetAtTime(myBoat && !myBoat.sunk ? .018 + localSpeed * .006 : 0, now, .08);

    const remoteDistance = me && otherBoat ? distance(me, otherBoat) : 999;
    const remoteSpeed = Math.abs(otherBoat?.speed || 0);
    const audible = otherBoat && remoteDistance < 120;
    this.remoteEngine.frequency.setTargetAtTime(48 + remoteSpeed * 4, now, .08);
    this.remoteGain.gain.setTargetAtTime(audible ? clamp(.12 * (1 - remoteDistance / 120), 0, .12) : 0, now, .08);
    this.remotePan.pan.setTargetAtTime(audible ? clamp((otherBoat.x - me.x) / 80, -1, 1) : 0, now, .08);
  }
}

const audio = new FreeAudio();
  ...
  socket.addEventListener("message", event => {
    ...
    if (message.type === "lobby-ready") {
      ...
      if (isHost) {
        ...
        announce("Свободных миров не было. Создано место ожидания капитана.");
      } else {
    ...
    if (message.type === "peer-connected") {
      if (isHost) {
        ...
        announce("Капитан мира подключён. Загружаю бухту…", true);
        send({type: "free-hello"});
      }
      return;
    }
    ...
    if (message.type === "network-closed") {
      announce("Второй игрок отключился. Мир остаётся доступен, пока открыт серверный игрок.", true);
    }
  });
...
function syncControlButtons() {
  ...
  $("repairButton").textContent = `Ремонт: ${localInput.repair ? "включён" : "выключен"}`;
}

function handleGameEvent(event) {
  audio.playEvent(event);
  if (event?.targets?.includes(playerIndex) && event.text) announce(event.text, ["sink", "ram", "tow-detach"].includes(event.type));
}
...
function render() {
  ...
  const myBoat = me.mode === "boat" || me.mode === "roof" ? world.boats[me.activeBoat] : null;
