...
export const SHOP_ITEMS = Object.freeze(base.SHOP_ITEMS.map(item => item.id === "mega-bomb-charge"
  ? Object.freeze({
      ...item,
      label: "заряды мега-бомбы",
      amount: MEGA_BOMB_BATCH,
      price: MEGA_BOMB_PRICE,
      maximum: MEGA_BOMB_MAXIMUM,
    })
  : item));
...
function stockFor(combat) {
  ...
  combat.megaBombAmmo = Math.min(COMPATIBILITY_MAGAZINE, combat.megaBombStock);
...
function description(world, playerIndex) {
  ...
  return `Заряды мега-бомбы. За покупку: ${MEGA_BOMB_BATCH}. Цена ${MEGA_BOMB_PRICE} кредитов. У тебя ${stockFor(combat)}. Максимум ${MEGA_BOMB_MAXIMUM}. Баланс команды ${state?.credits || 0}.`;
}
...
export function updateMerchantShop(world) {
  ...
  for (let index = 0; index < (world.players || []).length; index += 1) {
    ...
    if (current + MEGA_BOMB_BATCH > MEGA_BOMB_MAXIMUM) {
      emit(world, "shop-denied", `Покупка не помещается. Сейчас у тебя ${current}, максимум ${MEGA_BOMB_MAXIMUM}.`, [index]);
      ...
      combat.megaBombAmmo = Math.min(COMPATIBILITY_MAGAZINE, combat.megaBombStock);
      delete combat.megaBombReserve;
      emit(world, "shop-purchased", `Куплено 30 зарядов мега-бомбы. Теперь у тебя ${combat.megaBombStock}. Баланс команды ${state.credits}.`, [0, 1], {
        sourcePlayer: index,
