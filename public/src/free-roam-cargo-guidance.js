...
function arrivalText(player, target) {
  if (target.kind === "landing") {
    return "Двойной сигнал. Остановись у береговой высадки и выполни действие. Потом сонар поведёт к ящику.";
  }
  if (target.kind === "dock") {
    return "Двойной сигнал. Остановись у причала — груз разгрузится автоматически.";
  }
  if (player?.mode === "boat") {
    return "Двойной сигнал. Остановись и выполни действие, чтобы погрузить ящик на лодку.";
  }
  return "Двойной сигнал. Выполни действие, чтобы взять ящик в руки.";
}
